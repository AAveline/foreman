...
program.option("-f, --file <path>", "dependencies.json path");
...
const projects = JSON.parse(
  fs.readFileSync(program.file).toString()
) as Project;
const bar = new ProgressBar("parsing dependencies [:bar] :percent", {
...
for (const [project, path] of Object.entries(projects)) {
  ...
  const { dependencies } = JSON.parse(
    exec("npm list --json", { silent: true }) as any
  );
  ...
  for (const [key, val] of Object.entries(dependencies)) {
    ...
    if (computedVersion) {
      const [major, minor, patch] = computedVersion
        .replace(/^(\^)?(>=)?(\*)?/, "")
        .split(".");
    ...
    allDependencies[key] = allDependencies[key]
      ? [...allDependencies[key], [project, finalVersion, computedVersion]]
...
Object.entries(diff).forEach(([dependency, values]) => {
  ...
  const sorted = Object.entries(values).sort(
    ([, [, versionA]], [, [, versionB]]) => Number(versionB) - Number(versionA)
  );
